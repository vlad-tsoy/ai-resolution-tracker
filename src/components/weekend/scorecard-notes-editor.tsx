...
import { saveScorecardNotes } from "@/lib/actions";
import { useDebouncedCallback } from "@/lib/hooks/use-debounce";
import { Textarea } from "@/components/ui/textarea";
import Markdown from "react-markdown";

type ScorecardNotesEditorProps = {
  weekendId: number;
  initialNotes: string | null;
};

export function ScorecardNotesEditor({
  weekendId,
  initialNotes,
}: ScorecardNotesEditorProps) {
  ...
  const save = useCallback(
    async (text: string) => {
      ...
      await saveScorecardNotes(weekendId, text);
      setSaveStatus("saved");
  ...
  return (
    <section className="space-y-3">
      ...
      <Textarea
        ...
        placeholder="Best prompt, approach, what didn't work... (markdown supported)"
        rows={4}
        className="resize-y"
      />
