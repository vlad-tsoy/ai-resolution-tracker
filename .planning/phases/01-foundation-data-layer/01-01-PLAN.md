...
<task type="auto">
  ...
  <verify>
    ...
    Programmatically verify all four tables exist by running a SQL query via drizzle:
    ```bash
    npx tsx -e "
      ...
      db.execute(sql\`SELECT table_name FROM information_schema.tables WHERE table_schema = 'public' ORDER BY table_name\`)
        .then(rows => {
          ...
          const expected = ['done_criteria', 'scorecard_ratings', 'weekends', 'work_items'];
          const missing = expected.filter(t => !tables.includes(t));
          if (missing.length) { console.error('MISSING tables:', missing); process.exit(1); }
          console.log('All 4 tables verified:', tables.filter(t => expected.includes(t)).join(', '));
        });
    "
    ```
    All four tables (weekends, work_items, done_criteria, scorecard_ratings) must be listed.
  </verify>
  <done>
